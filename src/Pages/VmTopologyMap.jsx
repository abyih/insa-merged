    ...
    <div className="max-w-7xl mx-auto space-y-6 text-zinc-100">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            Cross-correlates identity across OpenStack (Nova / Neutron), OVS TAP interfaces, and OpenDaylight (br-int).
          </p>
        </div>
        <button
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center p-16 bg-zinc-900 border border-zinc-800 rounded-xl text-zinc-400 gap-3">
          ...
          <span className="text-sm">Querying OpenStack compute nodes and ODL bridge inventory…</span>
        </div>
      )}
      ...
      {data && (
        <>
          <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-zinc-800 bg-zinc-900 text-xs text-zinc-400 shadow-sm">
            ...
            <span>Active OpenDaylight Bridge Node:</span>
            ...
            {(data.mapping || []).map((row) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="text-zinc-500 font-semibold uppercase tracking-wider text-[10px]">ODL Node-Connector</span>
                    <span className="text-zinc-300 font-mono break-all bg-zinc-950 p-1 rounded border border-zinc-800">
                      {row.odlNodeConnectorId || "—"}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-2 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  >
                    ODL Inventory ({row.odlVisible ? "Linked" : "Unmapped"})
                  </span>
                </div>
              </div>
            ))}
