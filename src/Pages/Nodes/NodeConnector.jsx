...
// 	// --- FIX: Remove 'fetchDetail' from dependency array to prevent the infinite render loop ---
// 	useEffect(() => {
// 		let cancelled = false;
		
// 		const tick = () => { 
// 			if (!cancelled) {
// 				// Fetch detail in background without forcing a full-page screen wipe
// 				fetchDetail(true); 
// 			}
// 		};
...
// 			clearInterval(id); 
// 		};
// 	}, [nodeId]); // ONLY listen to nodeId

// 	const node = raw ? mapNodeDetails(raw) : null;

// 	// Only show full-page Loading spinner if we don't have any raw data yet
// 	if (loading && !raw) return <div className="p-8 text-center text-lg">Loading device details...</div>;
// 	if (error && !raw) return <div className="p-8 text-center text-red-600">Error: {error}</div>;
...
import FlowForm from "../../Components/Nodes/FlowForm";
import { useNodeDetail } from "../../pipeline/DataPipelineContext";
import { installFlow } from "../../api/apiController"; // Import the centralized flow installer
...
const NodeConnector = () => {
	...
	// --- FIX 1: Lift the 'formOpen' state to the parent so the timer can see it ---
	const [formOpen, setFormOpen] = useState(false);

	// --- FIX 2: Pause automatic background polling when the Flow Creation Form is open ---
	useEffect(() => {
		let cancelled = false;
		
		const tick = () => { 
			// Only fetch in the background if the user is not actively typing inside the flow form
			if (!cancelled && !formOpen) {
				fetchDetail(true); 
			}
		};
	...
	}, [nodeId, formOpen]); // Re-run interval setup when nodeId or formOpen state changes
	...
	// Pass formOpen state down to NodeDetails as a prop
	return <NodeDetails node={node} formOpen={formOpen} setFormOpen={setFormOpen} />;
};

export default NodeConnector;

// --- FIX 3: Receive formOpen and setFormOpen as props from parent ---
const NodeDetails = ({ node, formOpen, setFormOpen }) => {
	const [expandedFlows, setExpandedFlows] = useState({});
	const [activeSort, setActiveSort] = useState({
		column: "priority",
		asc: true,
	...
	// const [formOpen, setFormOpen] = useState(false); // REMOVED local state to use lifted props
	const [inactiveSort, setInactiveSort] = useState({
		column: "id",
		asc: true,
	});
	...
	const handleFormSubmit = async (data) => {
		...
		try {
			await installFlow(data.nodeId, data.tableId, data.flowId, flowBody);
			alert("Flow created successfully");
			setFormOpen(false); // Close the form on success
		} catch (err) {
			console.error(err);
