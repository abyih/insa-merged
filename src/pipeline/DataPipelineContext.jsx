...
import { createContext, useContext, useReducer, useEffect, useCallback } from "react";
import { cache } from "./cache";
import { getNodes, getNodeConnectors, getConnectionStats } from "../api/apiController";
import { mapNodes } from "../mappers/nodes-mapper";
import { mapFlowStats } from "../mappers/flow-stats-mapper";
import NetworkTopologySvc from "../Pages/Topology/TopologyService";
...
export function DataPipelineProvider({ children }) {
  ...
  // ── Node detail: smooth on-demand polling (no flickering) ────────────────
  const fetchNodeDetail = useCallback(async (nodeId, force = false) => {
    const KEY = `nodeDetail:${nodeId}`;
    ...
    if (!force) {
      const hit = cache.get(KEY);
      if (hit) { 
        dispatchDetail({ type: "SUCCESS", key: nodeId, payload: hit }); 
        return hit; 
      }
    }

    // 2. Only dispatch "LOADING" if we don't already have some data for this node
    const existingSlice = detailMap[nodeId];
    if (!existingSlice || !existingSlice.data) {
      dispatchDetail({ type: "LOADING", key: nodeId });
    }
