...
function readJsonSafe(file, defaultValue) {
  try {
    ...
    console.warn(`[Store] Could not read ${file}, using default:`, e.message);
  }
  return defaultValue;
}
...
const app = express();
const port = process.env.ONOS_SERVER_PORT || 5051;

// ONOS Controller Configuration
const ONOS_URL = process.env.ONOS_URL || "http://localhost:8183";
...
/* ==============================================================================
   PERSISTENCE: ONOS SLICES & CAPACITY ENDPOINTS
   ============================================================================== */
...
app.get(["/api/onos/ping", "/api/ping"], async (req, res) => {
  try {
    ...
    res.status(503).json({
      ...
      error: err.message,
      status: "DISCONNECTED",
      hint: `Ensure ONOS is running at ${ONOS_URL} and credentials are ${ONOS_USERNAME}:${ONOS_PASSWORD}`,
    });
  }
});
...
/* ==============================================================================
   LOCAL NEURAL INTENT SERVICE PROXY
   ============================================================================== */
...
app.get(["/api/onos/intent/health", "/api/intent/health"], async (req, res) => {
  try {
    ...
    res.status(503).json({
      ...
      hint: "Start it with: python3 anomaly/intent_service.py or use cloud AI providers",
    });
  }
});
