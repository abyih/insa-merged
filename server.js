...
app.put("/api/slices/:id", async (req, res) => {
  try {
    ...
    // Reconcile the Neutron DSCP marking rule with the new latency
    // requirement, since the network-level mark (unlike the OVS priority
    // queue) is applied once at the policy level and otherwise never gets
    // added or removed on update.
...
app.delete("/api/openstack/vms/:id", async (req, res) => {
  const { id } = req.params;
